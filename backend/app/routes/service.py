...
async def start_set(payload: ServiceSetStart, db: AsyncSession = Depends(get_db)):
    ...
    # displayed" tracking and the reading queue, when there wasn't an active
    # one already. Otherwise updating the translation mid-service would
    # silently wipe the preacher's in-progress reading queue.
