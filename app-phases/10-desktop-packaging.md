...
Exactly the "first run surfaces something" pattern every other phase's live testing has hit, five rounds in a row:
...
5. **Past the torch fix, Linux still hung indefinitely — this time after successfully bundling `.deb` and `.rpm`, right before AppImage.** A known Tauri-on-Linux-CI issue: `"targets": "all"` bundles AppImage too, and its `linuxdeploy` tooling tries to self-mount via FUSE by default. GitHub's Ubuntu runners don't have FUSE available, so instead of erroring out it just hangs forever waiting for a mount that will never happen — Windows and macOS have no AppImage-equivalent step, which is why only Linux ever hit this. Fixed with the documented workaround: `APPIMAGE_EXTRACT_AND_RUN: "1"` at the job level, telling the tooling to extract-and-run instead of mount (harmless no-op on the other two platforms).

All five are now fixed in the workflow and, where relevant, in the local setup docs too. **The workflow still hasn't completed a clean end-to-end run on all three platforms in the same attempt** — worth re-triggering (`workflow_dispatch`) to confirm, since each fix so far has been reactive to a real failure one at a time, not something verified in advance.
